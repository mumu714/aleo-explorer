...
async def finalize_execute(db: Database, cur: psycopg.AsyncCursor[dict[str, Any]], finalize_state: FinalizeState,
    ...
    if len(transition.outputs) != 0:
        ...
        if isinstance(maybe_future_output, FutureTransitionOutput):
            ...
            try:
                operations.extend(
                    await execute_finalizer(db, cur, finalize_state, async_order, set(), program, future.function_name, inputs, mapping_cache, local_mapping_cache, allow_state_change, dynamic_future_map=dynamic_future_map)
                )
            except ExecuteError as e:
...
async def finalize_block(db: Database, cur: psycopg.AsyncCursor[dict[str, Any]], block: Block) -> list[Optional[str]]:
    ...
    for confirmed_transaction in block.transactions.transactions:
        ...
        await execute_operations(db, cur, filter(lambda x: x["type"] != FinalizeOperation.Type.InitializeMapping, operations))
        reject_reasons.append(reject_reason)
    return reject_reasons


async def execute_operations(db: Database, cur: psycopg.AsyncCursor[dict[str, Any]], operations: Iterable[dict[str, Any]]):
